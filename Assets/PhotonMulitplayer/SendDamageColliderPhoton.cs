...
public class SendDamageColliderPhoton : MonoBehaviour {
	...
	HealthControllerPhoton myHealthController;
	HealthControllerPhoton targetHealthController;
	
	// Use this for initialization
	void Start () {
		myCharacter = transform.parent;
		myHead = myCharacter.Find("Head");
		myHealthController = myHead.GetComponent<HealthControllerPhoton>();
				...
				if(other.gameObject.tag != null)
				{
					if(other.gameObject.tag == targetTag)
					{
						...
						if(myCharacter.rigidbody2D.velocity.y < 0)
						{
							...
							targetHead.GetComponent<HealthControllerPhoton>().ApplyDamage(damageValue,true);
